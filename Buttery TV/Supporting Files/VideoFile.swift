...
//  MediaPlayer
//
//  Created by Jonathan Holland on 7/1/21.
//
...
/// A view that displays an environment-dependent videoPlayer.
///
/// The videoPlayer element on iOS is a wrapper of the AVPlayerViewController, while on macOS is a wrapper
/// around AVPlayerView
...
public struct VideoFile {
    ...
    /**
     * this determines if we need to start the videoPlayer at a specific seconds point in time.  It resets that value if it's not zero.
     */
...
#if os(iOS)
extension VideoFile: UIViewControllerRepresentable {
...
#elseif os(macOS)
extension VideoFile: NSViewRepresentable {
    
    public func makeNSView(context: Context) -> AVPlayerView {
        let videoView = AVPlayerView()
        videoView.player = AVPlayer(url: videoURL)
        videoView.showsSharingServiceButton = true
    ...
    public class VideoCoordinator: NSObject {
        ...
        var player: AVPlayer? {
            didSet {
                ...
                NotificationCenter.default.addObserver(self,
                                                       selector:#selector(VideoFile.VideoCoordinator.playerItemDidReachEnd),
        ...
        @objc public func playerItemDidReachEnd(notification: NSNotification) {
            if videoPlayer.loop.wrappedValue {
                ...
                videoPlayer.startVideoAtSeconds.wrappedValue = 0.0
        ...
        @objc public func updateStatus() {
            if let player = player {
                ...
                videoPlayer.lastPlayInSeconds.wrappedValue = playerCurrentTime
            } else {
                videoPlayer.isPlaying.wrappedValue = false
            }
        }
        ...
        func seekOnStartToSecondsIfNeeded(startVideoAtSeconds:Double?) {
            if let startVideoAtSeconds = startVideoAtSeconds {
                ...
                DispatchQueue.main.async {
                    ...
                    self.videoPlayer.startVideoAtSeconds.wrappedValue = 0.0
                }
            }
        }
        ...
        func seekForward(forwardInSeconds: Double) {
            ...
            DispatchQueue.main.async {
                ...
                self.videoPlayer.forwardInSeconds.wrappedValue = 0.0
            }
        }
        ...
        func seekBackward(backInSeconds: Double) {
            ...
            DispatchQueue.main.async {
                // resets the value to be tapped again.
                self.videoPlayer.backInSeconds.wrappedValue = 0.0
            }
        }
        ...
        override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
            ...
            if let player = player {
                #if os(macOS)
                DispatchQueue.main.async {
                    self.videoPlayer.isMuted.wrappedValue = player.volume == 0
                }
                #else
                DispatchQueue.main.async {
                    self.videoPlayer.isMuted.wrappedValue = player.isMuted
                }
                #endif
            }
        }
...
extension VideoFile {
    ...
    public func pictureInPicturePlayback(_ value:Bool) -> VideoFile {
        var new = self
        new.allowsPictureInPicturePlayback = value
        return new
    }
    
    public func playbackControls(_ value: Bool) ->VideoFile {
        var new = self
        new.showsPlaybackControls = value
        return new
    }
    
    public func isMuted(_ value: Bool) -> VideoFile {
        return isMuted(.constant(value))
    }
    
    public func isMuted(_ value: Binding<Bool>) -> VideoFile {
        var new = self
        new.isMuted = value
        return new
    }
    
    public func isPlaying(_ value: Bool) -> VideoFile {
        let new = self
        new.isPlaying.wrappedValue = value
        return new
    }
    
    public func isPlaying(_ value: Binding<Bool>) -> VideoFile {
        var new = self
        new.isPlaying = value
        return new
    }
    
    public func lastPlayInSeconds(_ value: Binding<Double>) -> VideoFile {
        var new = self
        new.lastPlayInSeconds = value
        return new
    }
    
    public func backInSeconds(_ value: Binding<Double>) -> VideoFile {
        var new = self
        new.backInSeconds = value
        return new
    }
    
    public func forwardInSeconds(_ value: Binding<Double>) -> VideoFile {
        var new = self
        new.forwardInSeconds = value
        return new
    }
    
    public func videoGravity(_ value: AVLayerVideoGravity) -> VideoFile {
        var new = self
        new.videoGravity = value
        return new
    }
    
    public func loop(_ value: Bool) -> VideoFile {
        self.loop.wrappedValue = value
        return self
    }
    
    public func loop(_ value: Binding<Bool>) -> VideoFile {
        var new = self
        new.loop = value
        return new
    }
